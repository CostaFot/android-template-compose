...
import com.feelsokman.androidtemplate.rememberscope.ScopedViewModelContainer.ExternalKey
import com.feelsokman.androidtemplate.rememberscope.ScopedViewModelContainer.InternalKey
            ...
            if (scopedObjectKeys.containsKey(positionalMemoizationKey)
                ...
                scopedObjectsContainer.remove(positionalMemoizationKey) // Remove in case key changed
                    ?.also { // Old object may need to be cleared before it's forgotten
                        clearLastDisposedObject(
                            disposedObject = it,
                            objectsContainer = scopedObjectsContainer.values.toList()
                        )
                    }
                scopedObjectKeys[positionalMemoizationKey] =
                    externalKey // Set the new external key used to track and store the new object version
