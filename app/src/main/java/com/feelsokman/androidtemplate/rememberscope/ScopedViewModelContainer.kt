...
public class ScopedViewModelContainer : ViewModel(), LifecycleEventObserver {
    ...
    internal fun <T : Any> getOrBuildObject(
        ...
        return if (scopedObjectKeys.containsKey(positionalMemoizationKey) && (scopedObjectKeys[positionalMemoizationKey] == externalKey)) {
            ...
            scopedObjectKeys[positionalMemoizationKey] =
                externalKey // Set the external key used to track and store the new object version
    ...
    public fun <T : ViewModel> getOrBuildViewModel(
        ...
        val factory =
            if (owner is HasDefaultViewModelProviderFactory) owner.defaultViewModelProviderFactory else ViewModelNewInstanceFactory.instance
    ...
    internal fun <T> onDisposedFromComposition(keyInScopeResolver: KeyInScopeResolver<T>) {
        scopedObjectKeys.forEach { (key, externalKey: ExternalKey) ->
            val scopeKeyWithResolver: ScopeKeyWithResolver<*>? =
                externalKey.scopeKeyWithResolver() // Get the KeyInScopeResolver if ExternalKey is one
     ...
     */
    private fun clearLastDisposedObject(
        disposedObject: Any,
        objectsContainer: List<Any> = scopedObjectsContainer.values.toList()
    ) {
    ...
    override fun onStateChanged(source: LifecycleOwner, event: Lifecycle.Event) {
        when (event) {
            ...
            Lifecycle.Event.ON_DESTROY -> { // Remove ourselves so that this ViewModel can be garbage collected
                ...
                compositionResumedTimeout =
                    CountDownLatch(1) // Start a new latch for the next time this ViewModel is used after resume
            }
